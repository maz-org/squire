...
export const MonsterStatSchema = z.object({
  ...
  normal: z.record(z.string(), MonsterLevelStats).describe('Normal difficulty stats keyed by level number string'),
  elite: z.record(z.string(), MonsterLevelStats).describe('Elite difficulty stats keyed by level number string'),
...
export const MonsterAbilitySchema = z.object({
  ...
  abilities: z.array(z.string()).describe('Each ability line verbatim, describing icons as words (e.g. "Attack +1", "Muddle")'),
});
...
export const CharacterAbilitySchema = z.object({
  ...
  level: z.union([z.number().int(), z.literal('X')]).nullable().describe('Card level number, "X" for lost cards with no level, or null'),
...
export const ItemSchema = z.object({
  ...
  slot: z.enum(['head', 'body', 'legs', 'one hand', 'two hands', 'small item']).describe('Equipment slot'),
...
export const EventSchema = z.object({
  ...
  }).nullable().describe('Option B, or null if there is no choice'),
});
