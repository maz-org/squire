...
2. **Flexible main surface** — occupies the bulk of the viewport. The
   ...
   (`<footer class="squire-toolcall">`) inline — single line in `--sepia`
   small caps, "CONSULTED · RULEBOOK · SCENARIO BOOK" or similar — so
   provenance travels with the answer it sourced rather than living as
   page chrome. Phase 5 hosts two card mockups side-by-side with a
...
Same tokens, different arrangement. The main content column stays 640px wide,
centered within the remaining viewport. Per
[ADR 0012](docs/adr/0012-split-home-and-scrolling-chat-ia.md), Phase 1 does
**not** render a desktop left rail — the rail aside is reserved for Phase 4
character state and Phase 5 card comparison, which will return as either a
side panel, modal, or their own routes (not as peer modes of the
...
| 2026-04-08 | **No emoji in tool indicators or anywhere else.** Tool-start renders as `--sepia` Geist 10–11px small-caps `CONSULTING · RULEBOOK` while a tool is in flight; once prose resumes the transient row is cleared and provenance carries on the per-answer footer (`CONSULTED · RULEBOOK · CARD INDEX`, no page numbers).                                                                                                                               | The Modern Codex aesthetic does not allow magnifying-glass glyphs. The sepia small-caps treatment is the same vocabulary as the tool-call footer line, so the streaming indicators collapse into the footer naturally. Earlier SQR-8 draft used 🔎 — flagged in plan-design-review Pass 4. Original row referenced a `P.47` page-number suffix; SQR-105 dropped that — the consulted footer now lists rulebook section labels, not page numbers. |
...
| 2026-04-22 | **Split home + scrolling-chat IA — supersedes 2026-04-08 current-turn ledger** — `/chat/:id` is a standard scrolling-chat transcript with the drop cap on the newest answer only (position-based selector). `/` is a separate purpose-built landing. The Phase 1 desktop rail and the recent-questions chip row are deleted on both surfaces.                                                                                                       | The 2026-04-08 current-turn ledger was a novel IA users had to learn; lived-use surfaced four problems (fake home chips, no first-turn history affordance, orphaned selected-message surface, scaffolding desktop rail). Position-based drop cap rarity preserves the signature without forcing one-turn-at-a-time. From SQR-101 / [ADR 0012](docs/adr/0012-split-home-and-scrolling-chat-ia.md).                                                |
