...
this document. v0.6 (2026-06-04) preserves the split home + scrolling-chat IA
and adds a real conversation-history rail/drawer per
[ADR 0020](docs/adr/0020-conversation-history-rail.md); the drop cap appears
only on the newest answer (position-based selector), and live-region duties move
from `main.squire-surface` to `section.squire-transcript`.
...
The main content column stays 640px wide within the remaining viewport. Per
[ADR 0020](docs/adr/0020-conversation-history-rail.md), this rail is real
conversation history, not the old campaign/party placeholder rail deleted by
ADR 0012. Phase 4 character state and Phase 5 card comparison return later as a
right-side context/progress panel, modal, or their own routes.
