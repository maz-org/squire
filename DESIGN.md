...
**Version:** 0.15
**Date:** 2026-06-27
**Status:** Approved via `/design-consultation`. Covers all seven phases of the
...
- **Project types the system must serve:** mobile web chat (Phase 1), card /
  ...
  spoiler filtering and visual polish (Phase 6), shareable recommendation
  exports (Phase 6), and cross-channel identity for additional assistants
  (Phase 7).
...
(Phase 4), inventory grids (Phase 5), status surfaces, and shareable exports
as equal first-class modes, not "the chat view with attachments bolted on."
...
Used as favicon, sign-in page mark, mobile header, desktop header, and future
channel avatars. This is Squire's face — keep it consistent across all
surfaces.
...
Use a short saved-state label such as "SAVED", "UPDATED", or "CONFIRMED" plus
a short body describing the local change. Do not imply external import or
recurring sync.
...
## Cross-channel Identity (Phase 7)
...
- One wordmark + monogram for future channel inheritance.
...
2. **Oracle-layout decision was rejected.** An earlier proposal had the
   ...
   dramatic but serves the full roadmap.
...
[docs/artifacts/campaign-workspace-redesign/party-approved-variant-a.png](docs/artifacts/campaign-workspace-redesign/party-approved-variant-a.png)
...
users can have multiple live campaigns at once, so no workspace control should
imply a single active campaign.
...
| Manual unlock source | `--sepia` label such as `EVENT`, `PERSONAL QUEST`, `RANDOM`, or `PUZZLE`, with condition note |
| Missing graph data   | Plain note explaining recorded scenario keys that Squire cannot place on the graph yet        |
...
- **Workspace header**: campaign name is the strongest first-viewport signal.
  ...
  and prosperity only; open scenario counts belong in the Progress surface.
  Do not make page-level navigation a stylized small-caps label that only reads
  as a link on hover.
...
- **Stats line** under the header: `PLAYED 16 · UNLOCKED 7 · LOCKED 58 ·
...
- **Record progress** is the primary action. Derived statuses recalculate via
  HTMX swap with a fixed, auto-dismissing `aria-live="polite"` toast
  ("Scenario 19 marked played."). Rows with hazard consequences always confirm
  before applying.
...
The edit surface uses structured, always-visible sheet panels (identity,
progress, gold, perks, items, cards, personal quest, notes). Do not render these
as summary bars or accordion rows. The page should read like a digital character
record loosely inspired by the paper sheet: compact core-field panels, an XP
track, checklist rows for perks, ledger rows for carried items/cards, and
separate private panels for quest and notes. Each panel still edits in place and
keeps a **deep-linkable anchor** (`#gold`, `#progress`, `#items`) so agent
work-log rows and validation warnings can link "fix it here." Level is derived
from XP, perks come from the class mat checklist, ability cards come from the
character's class list, items and personal quests come from campaign-managed
catalogs, and battle goals are omitted until there is a session-scoped model.
Non-owners see member-visible fields only; unclaimed placeholders show a claim
banner (`.squire-banner` sage variant). Empty sections say "not recorded" with
an add affordance -- never fake values. See
...
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-04-08 | Design system created via `/design-consultation`                                                                                                                                                                                                                                                                                                                                                                                                                         | Covers the full roadmap, not just Phase 1 rules Q&A.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 2026-04-08 | **Metaphor: attendant/ledger, not oracle**                                                                                                                                                                                                                                                                                                                                                                                                                               | Name "Squire" is literal. Serves campaign state, recommendations, shareable outputs, and additional channels.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| 2026-04-08 | **Dark mode is unconditional in Phase 1.** `prefers-color-scheme` is NOT honored. Light mode tokens exist via `[data-theme="light"]` for a future user toggle                                                                                                                                                                                                                                                                                                            | Per SPEC's phone-at-the-table primary surface and DESIGN.md §Color "Dark mode is the default." A user-controlled toggle is the right path; auto-flipping in Phase 1 would surprise users in dim rooms. From plan-design-review Pass 6.                                                                                                                                                                                                                                                                                                                                                                                       |
...
| 2026-06-12 | **G5: Phase 4 component vocabulary** — sepia small-caps status labels (PLAYED sage / UNLOCKED parchment / LOCKED sepia-dim / BLOCKED amber / manual unlock source sepia), Fraunces thread headings + sepia notes, tabular-nums stats line, adjacent `.squire-banner` amber hazard warnings, confirmation block = surface panel + work-log rows + wax confirm (distinct from the Phase 5 verdict block)                                                                   | Reuses the existing token and banner vocabulary instead of inventing new chrome; statuses as typography (not chips) keeps the printed-index feel and answered the variant-A "badges too loud" feedback. SQR-359 manual testing collapsed the old visible `VIA EVENT` / `DREW IT` states: once a scenario is unlocked, the table sees it as unlocked regardless of source. See §Phase 4 Components.                                                                                                                                                                                                                           |
...
| 2026-06-28 | **G13: Character sheet panels, not summary rows** — narrows G12. The structured controls render as a paper-sheet-inspired two-column workspace with compact core metrics, an XP track, checklist rows, and ledger rows.                                                                                                                                                                                                                                                  | The first structured implementation removed native accordions but kept the old summary-row visual language, which still felt like an accordion. The panel layout preserves the same anchors and no-JS forms while making the character page read like an actual sheet.                                                                                                                                                                                                                                                                                                                                                       |
...
- **2026-06-28 (v0.15):** Character sheet controls now render as fixed
  paper-sheet-inspired panels instead of accordion-style summary rows. Anchors,
  no-JS form posts, and private projection are preserved.
...
- **2026-06-19 (v0.13):** Campaign workspace redesign supersedes the
  ...
  tabs, a breadcrumb-based workspace header, active and retired characters, Players-owned
...
- **2026-04-08 (v0.3):** Finalized via `/design-consultation`. Approved
  system covers the full roadmap, not just Phase 1.
