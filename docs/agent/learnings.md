...
- **env-example-should-follow-code-not-doc-snippets** (pattern): When setup docs drift, treat `process.env` usage in `src/` and scripts as the source of truth for supported configuration, then update `.env.example` and the docs from that inventory. Doc snippets tend to fossilize first. Files: `.env.example`, `src/server.ts`, `src/auth/google.ts`, `src/auth/session-middleware.ts`, `src/instrumentation.ts`, `src/db.ts`, `src/worktree-runtime.ts`, `src/ghs-utils.ts`. Source: `observed`.
- **merged-pr-branch-may-still-have-unmerged-local-commits** (operational): A merged PR and a safely deletable local branch are not the same thing. Squash merges and post-merge follow-up commits can leave local-only work behind, so check `main..branch` before `git branch -d` instead of assuming "PR merged" means "branch fully merged." Files: `docs/agent/review.md`, `docs/agent/shipping.md`. Source: `observed`.
