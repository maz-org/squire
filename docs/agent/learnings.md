...
- **browse-commands-must-be-sequential** (operational): During /qa, running multiple gstack browse CLI commands in parallel can race on .gstack/browse.json and produce ENOENT state-file errors. Run browse commands sequentially, especially goto/snapshot/screenshot chains. Files: `.gstack/browse.json`. Source: `observed`.
