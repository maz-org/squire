...
- **campaign-start-building-costs** (pattern): For campaign-start Frosthaven buildings such as Craftsman and Alchemist, keep the initial build cost and the printed level-card upgrade cost as separate data fields. The level 1 card can show the cost to upgrade to level 2 while the building itself starts already built with no initial construction cost. Files: `src/import-buildings.ts`, `data/extracted/buildings.json`, `src/extracted-data.ts`. Source: `observed`.
...
- **langfuse-trace-urls-must-come-from-htmlpath** (operational): Do not hand-build Langfuse trace UI URLs. Filtered Anthropic evals can create both a deterministic `eval.case` trace ID for replay/API use and an SDK `experiment-item-run` trace with a short hex ID that is safer for browser links. Query the Langfuse API and use the returned `htmlPath`; share the `experiment-item-run` URL for UI viewing when present. Files: `docs/CONTRIBUTING.md`, `eval/replay.ts`. Source: `observed`.
