...
3. **Views** (`layoutShell`, `renderAuthErrorPage`, `renderHomePage`) accept `session?: Session`. Session present = logged in = the view renders its surface-specific interaction chrome (every authenticated surface ships the input dock). Per [ADR 0020](adr/0020-conversation-history-rail.md), the home page is a focused landing with conversation history in the app shell, and the conversation page is a scrolling transcript with the same history shell. Session absent = logged out = brand-only chrome (header, monogram). Views never import from auth modules.
...
Squire targets multiple games in the \*haven family. Today, the rules Q&A agent supports Frosthaven and Gloomhaven (2nd Edition). Campaign tracking has a broader content registry: Gloomhaven 1e can include 1e solos and Jaws of the Lion, Jaws can stand alone, GH2e can include GH2e solos, and Frosthaven can include FH solo scenarios. Tracker-only games are not valid knowledge-agent `GameId`s until their rules corpus, static data, and eval coverage are added.
