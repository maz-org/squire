...
**Web channel: Google OAuth + Postgres sessions** (`src/auth/google.ts`, `src/auth/session-middleware.ts`, `src/auth/csrf.ts`). Squire acts as an OAuth **client**, redirecting users to Google for consent. The callback verifies the ID token via `google-auth-library`, checks a hard-coded email allowlist (Phase 1, ADR 0009), upserts the user, and creates a server-side session in the `sessions` table. Session cookie: HttpOnly, Secure (production only), SameSite=Strict, signed via `SESSION_SECRET`. 30-day expiry matching the long-lived token DX policy. PKCE state and code_verifier are stored in a short-lived signed cookie during the OAuth flow. Mutating web routes use a per-session CSRF token derived from the session ID and `SESSION_SECRET`, rendered into authenticated HTML and attached to HTMX requests via inherited `hx-headers`.
