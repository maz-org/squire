...
| `Squire production uptime failure`              | Uptime monitor  | URL `https://squire.maz.org/api/health`; successful HTTP response; 10s timeout; no JSON assertion      | 5 consecutive failed 60s checks; recovery after 2 good checks | Check Sentry failure reason/status, then compare `/api/health`, `/api/live`, Fly | `npm run sentry:test-event -- --kind uptime`; use monitor test for alerting |
