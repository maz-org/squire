...
| `concept`       | A curated game term — condition, keyword, or core mechanic — with rulebook definition, FAQ clarifications, and card references reachable as knowledge-graph edges | `knowledge_concepts` + `knowledge_edges` (ADR 0027) | `concept:frosthaven/muddle`, `concept:gloomhaven-2e/retaliate`                                                                                         |
...
- Concept edges (provenance `concepts`, SQR-402) are derived
  deterministically at seed time: `defines` points from the best-matching
  rulebook chunk, `clarifies` from every FAQ/errata chunk mentioning the
  term, and `references` from cards whose printed text mentions it.
