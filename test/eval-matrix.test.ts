...
describe('eval matrix runner', () => {
  ...
  it('surfaces groundedness pass/fail and failure details in reports', async () => {
    const runner: EvalMatrixRunner = vi.fn(async ({ traceId, traceUrl }) => ({
      ok: true,
      answer: 'correct but unsupported',
      traceId,
      traceUrl,
      score: 1,
      pass: true,
    ...
    expect(formatEvalMatrixTable(result.rows)).toContain('groundedness_failures');
    expect(formatEvalMatrixTable(result.rows)).toContain(
      'no source labels or canonical refs were recorded by successful tool calls',
    );
    expect(formatEvalMatrixMarkdown(result.rows)).toContain('groundedness failures');
  });
